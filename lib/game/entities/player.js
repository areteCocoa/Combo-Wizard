...
EntityPlayer = ig.Entity.extend({
	...
	update: function() {
		if (this.manaTimer.delta() > 0) {
			...
			if(this.combatTimer.delta() > 0) {
				this.manaTimer.set(.05);
			} else {
				this.manaTimer.set(.5);
			}
		}
		...
		comboCheck = function(){
			...
			for (var count=0; count<spellEntitiesArray.length; count++) {
				if (spellEntitiesArray[count].indexOf(attackEntity) != -1) {
					var manaCost = 4*Math.pow(4, count);
				}
			}

			// logBase: how many levels required to half manaCost
			var logBase = 10;
			if (joinedElements.indexOf('q') != -1) manaCost = manaCost/((Math.log(ig.global.fireLevel)/Math.log(logBase)) + 1);
			if (joinedElements.indexOf('w') != -1) manaCost = manaCost/((Math.log(ig.global.waterLevel)/Math.log(logBase)) + 1);
			if (joinedElements.indexOf('e') != -1) manaCost = manaCost/((Math.log(ig.global.earthLevel)/Math.log(logBase)) + 1);
			manaCost = Math.floor(manaCost);

			return [attackEntity, manaCost];
		}
		...
		if (ig.input.state('r')) {
			...
			if ( attackEntity[0] ) {
				...
				if (ig.game.currentMana > manaCost && this.spellTimer.delta() > 0) {
					...
					ig.game.spawnEntity ( attackEntity[0], this.pos.x, this.pos.y, settings);
