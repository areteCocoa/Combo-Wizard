...
EntityPlayer = ig.Entity.extend({
	...
	update: function() {
		...
		if (ig.input.state('up') && ig.input.state('left')){
			this.projectileY = -150;
			this.projectileX = -150;
			console.log("UP LEFT");
		}
		...
		if (ig.input.state('down') && ig.input.state('right')){
			this.projectileY = 150;
			this.projectileX = 150;
			console.log("DOWN RIGHT");
		}
		if (ig.input.state('right') && !ig.input.state('up') && !ig.input.state('down')){
			this.projectileY = 0;
			this.projectileX = 150;
			console.log("RIGHT");
		}
		if (ig.input.state('left') && !ig.input.state('up') && !ig.input.state('down')){
			this.projectileY = 0;
			this.projectileX = -150;
			console.log("LEFT");
		...
		if (ig.input.state('r')) {
			...
			if ( attackEntity ) {
				...
				if (ig.game.currentMana > manaCost) {
					...
					var settings = {vel: {x: this.projectileX, y: this.projectileY}};
