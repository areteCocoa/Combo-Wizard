...
EntityPlayer = ig.Entity.extend({
	...
	update: function() {
		...
		if (ig.input.state('r')) {
			...
			if( attackEntity ) {
				ig.game.spawnEntity ( attackEntity, this.pos.x-12, this.pos.y-10, {vel: {x: (this.flip ? -150 : 150), y:1}, flip: this.flip});
			}
		}
...
entityQ = ig.Entity.extend({
		...
		gravityFactor: 0,
		animSheet: new ig.AnimationSheet('media/q.png', 7, 11),

		init: function( x, y, settings ) {
			this.parent( x, y, settings );
			this.addAnim( 'idle', 1, [0] );
			this.addAnim( 'spawn', .05, [0])
