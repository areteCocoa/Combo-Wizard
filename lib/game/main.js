...
MyGame = ig.Game.extend({
	...
	// Code to add global XP veriable here.

	levels: [LevelChunk0, LevelChunk1, LevelChunk2, LevelChunk3, LevelChunk4, LevelChunk5, LevelChunk6, LevelChunk7, LevelChunk8],
	...
	init: function() {
		...
		this.levels = [LevelChunk0, LevelChunk1, LevelChunk2, LevelChunk3, LevelChunk4, LevelChunk5, LevelChunk6, LevelChunk7, LevelChunk8];
	...
	update: function() {
        ...
        if ( player == undefined ){
        	...
        	this.maxMana = this.playerMaxMana;
        }
		this.parent();
	},
	...
	draw: function() {
		...
		if( player ) {
            ...
            this.manaImage.draw(5, 40);
            
            var manaPercent = ((this.currentMana*38 / this.playerMaxMana));
            for (var count = 0; count < manaPercent; count++ ) {
            	this.manaFillerImage.draw(count + 6, 41);
            }
			this.font.draw("Mana: " + this.currentMana, 5, 41, ig.Font.ALIGN.LEFT);
			this.font.draw("XP: " + this.xp, 5, 50, ig.Font.ALIGN.LEFT);
		...
		this.font.draw( 'ComboWizard v0.2', x, 0, ig.Font.ALIGN.RIGHT );
	...
	spawnPlayer: function() {
		...
		if (player == undefined && spawn != undefined) {
			...
			this.playerMaxMana = this.maxMana;
