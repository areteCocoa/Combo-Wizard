...
MyGame = ig.Game.extend({
	...
	draw: function() {
		...
		if( player ) {
            ...
            var manaPercent = ((player.mana*38 / player.maxMana));
            for (var count = 0; count < manaPercent; count++ ) {
            	this.manaFillerImage.draw(count + 6, 41);
            }
			this.font.draw("Mana: " + player.mana, 5, 41, ig.Font.ALIGN.LEFT);
	...
	spawnPlayer: function() {
		...
		if (player == undefined && spawn != undefined) {
			...
			this.player.mana = this.maxMana;
			this.player.maxMana = this.maxMana;
